...
export default function EditRulePage() {
  ...
  const loadRule = async () => {
    try {
      setLoading(true);
      const response = await adminApi.alertAutomation.getRuleById(ruleId);
      ...
      if (response.success && response.data) {
        ...
        if (rule.type === 'weather') {
          setTempMin(conditions.temperature?.min?.toString() || '');
          setTempMax(conditions.temperature?.max?.toString() || '');
          setPrecipMin(conditions.precipitation?.min?.toString() || '');
          setWindMin(conditions.windSpeed?.min?.toString() || '');
        } else {
          setMagMin(conditions.magnitude?.min?.toString() || '');
          setMagMax(conditions.magnitude?.max?.toString() || '');
          setDepthMax(conditions.depth_max?.toString() || '');
          setRadiusKm(conditions.radius_km?.toString() || '100');
        }

        // Load alert template
        const template = rule.alert_template;
        setAlertType(template.alert_type);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const response = await adminApi.alertAutomation.updateRule(ruleId, updates);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Alert Rule</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-1">Update conditions and alert template</p>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="px-3 py-2 bg-gray-100 rounded-md text-gray-700">
                {type === 'weather' ? '🌦️ Weather' : '🌍 Earthquake'}
